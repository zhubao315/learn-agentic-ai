...
2. **Lightweight Agents**  
   ...
   - **Choice**: You’re using the **OpenAI Agents SDK** to build these agents. This SDK offers a streamlined way to create lightweight agents with built-in features like memory management (LangMem integration) and tool usage.  
...
3. **REST APIs**  
   ...
   - **Choice**: **FastAPI** is our framework here. FastAPI is a high-performance, Python-based tool that supports asynchronous programming and auto-generates OpenAPI documentation, accelerating development.  
...
4. **Stateless Serverless Docker Containers**  
   ...
   - **Choice**: We’re using **Docker Containers**, which provide a lightweight, consistent runtime environment deployable across platforms.  
...
5. **Asynchronous Message Passing**  
   ...
   - **Choice**: **Upstash Kafka** is your serverless tool. Kafka is a distributed streaming platform optimized for high-throughput, fault-tolerant messaging, connecting agents in complex workflows.  
...
6. **Flexible Container Invocation**  
   ...
   - **Choice**: For prototyping, you’re using **cron-job.org**, a free online scheduling service. For production, you’re opting for **Kubernetes CronJob**, which integrates with Kubernetes for robust scheduling.  
...
7. **Relational Managed Database Services**  
   ...
   - **Choice**: **CockroachDB** is your selection. CockroachDB is a distributed SQL database compatiable with Postgres designed for scalability and resilience, with managed services to reduce operational burden.  
...
- **Scheduled Container Invocation**: Containers executed on a predefined schedule (via cron jobs) support batch processing or periodic tasks, adding flexibility to the system. They will also be used to **pull asynchronous messages from Kafka**.  
...
- **Asynchronous Message Passing**: Upstash Kafka (Free tier) 
